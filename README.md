...
**未上课的讲次不得开放给学生。** 开放状态由 `assets/progress.js` 中的 `window.DL_PROGRESS` 统一控制（每章 `open` / `locked`）：

1. 调整开放状态只改 `progress.js`，锁定后的呈现机制以仓库当前实现为准（遮罩式提示"本章尚未开放"），内容保留在仓库中
2. 首页卡片、章节页、slides、搜索索引的开放/锁定表现均跟随 `progress.js`，不要单独手工恢复某一章
3. 每上完一讲：把该章改为 `open` 并推送发布即可
4. **禁止把全部章节一次性恢复开放**，除非老师当次明确指示

> 2026-09-18 起生效：第 01、02 讲开放（02 仅前 4 节），第 03~06 讲锁定。
